...
export class CodeblogComponent {
  ...
  buildPackageJS = async () => {
    ...
    if (!_packageJSON) {
      ...
      this.setManifest(BuildStatus.failed);
      return;
    } else if (!_packageJSON.codeblog) {
      this.log(
        `${this.manifestPath} is missing "codeblog" in the default export.`,
        LogLevel.error
      );
      this.log(
        `This is required so Codeblog knows how to render your component`,
        LogLevel.error
      );
    ...
    this.setRegistration(_packageJSON.codeblog);
...
export class CodeblogComponentStore {
  static async load(
    packageName: string,
    folderPath: string,
    mode: "dev" | "release",
  ...
  ): Promise<CodeblogComponent> {
    const component = new CodeblogComponent({
      packageName,
  ...
  static async loadAll(
    ...
    log: LogFunction,
    sendComponentUpdate: Function
  ): Promise<Array<CodeblogComponent>> {
